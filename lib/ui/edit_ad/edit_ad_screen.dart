...
class _EditAdScreenState extends State<EditAdScreen> with ValidationMixin {
  ...
  void didChangeDependencies() {
    ...
    if (_initialRun) {
      ...
      _subList = _homeProvider.getCategoryList(categoryModel:  CategoryModel(isSelected:false ,catId: widget.ad.adsCat,catName:
      widget.ad.adsCatName,catImage: 'assets/images/all.png'),enableSub: true,catId:widget.ad.adsCat);
                      ...
                      GestureDetector(
                        ...
                        onTap: () async{
                          ...
                          await _apiProvider.post(
                              "https://matloobservices.com/api/do_delete_photo" +
                  ...
                  child: Column(
                    children: <Widget>[
                      ClipRRect(
                          borderRadius: BorderRadius.circular(8.0),
                          child: Image.network(
                            widget.ad.adsPhoto33!=null?widget.ad.adsPhoto33:"",
                            height: _width*.26,
                          )),
                      GestureDetector(
                        child: Text("حذف"),
                  ...
                  child:Column(
                    children: <Widget>[
                      ClipRRect(
                          borderRadius: BorderRadius.circular(8.0),
                          child: Image.network(
                            widget.ad.adsPhoto44!=null?widget.ad.adsPhoto44:"",
                            height: _width*.26,
                          )),
                      GestureDetector(
                        child: Text("حذف"),
                ...
                if (snapshot.hasData) {
                  if (snapshot.hasData) {
                    ...
                    if (_initSelectedCountry) {
                      ...
                      for (int i = 0; i < snapshot.data.length; i++) {
                        if (widget.ad.adsCityName == snapshot.data[i].countryName) {
    ...
    if(_imageFile != null) {
                  ...
                  formData = new FormData.fromMap({
                    ...
                    "imgURL[0]": await MultipartFile.fromFile(_imageFile.path,
                        filename: fileName),
                    "imgURL[1]": _imageFile1 != null ? await MultipartFile.fromFile(
                        _imageFile1.path,
                        filename: fileName1) : "",
                    "imgURL[2]": _imageFile2 != null ? await MultipartFile.fromFile(
                        _imageFile2.path,
                        filename: fileName2) : "",
                    "imgURL[3]": _imageFile3 != null ? await MultipartFile.fromFile(
                        _imageFile3.path,
                        filename: fileName3) : "",
                  });
